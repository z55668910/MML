...
class Augmentation:
    ...
    def _shift_scale_aug(self, image, bbox, crop_bbox, size, salient):
        im_h, im_w = image.shape[:2]
        ...
        image = self._crop_roi(image, crop_bbox, size)
        salient = self._crop_roi(salient, crop_bbox, size)
        return image, bbox, salient

    def _flip_aug(self, image, salient, bbox):
        image = cv2.flip(image, 1)
        salient = cv2.flip(salient, 1)
        width = image.shape[1]
        bbox = Corner(width - 1 - bbox.x2, bbox.y1,
                      width - 1 - bbox.x1, bbox.y2)
        return image, bbox, salient

    def __call__(self, image, bbox, size, gray=False, salient=None):
        shape = image.shape
        ...
        # if not salient is None:
        #     salient, _ = self._shift_scale_aug(salient, bbox, crop_bbox, size)

    
        # shift scale augmentation
        image, bbox, salient = self._shift_scale_aug(image, bbox, crop_bbox, size, salient)
        ...
        if self.flip and self.flip > np.random.random():
            image, bbox, salient = self._flip_aug(image, salient, bbox)
