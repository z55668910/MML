...
def build_opt_lr(model, current_epoch=0):
    ...
    trainable_params += [{'params': model.head_1.parameters(),
                          'lr': cfg.TRAIN.BASE_LR}]

    trainable_params += [{'params': model.head_2.parameters(),
                          'lr': cfg.TRAIN.BASE_LR}]
...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer):
    ...
    for idx, data in enumerate(train_loader):
        ...
        ########## 我加的程式碼 ########## 
        if epoch != idx // num_per_epoch + start_epoch or idx+1 == len(train_loader):
            epoch = epoch + 1 if idx+1 == len(train_loader) else idx // num_per_epoch + start_epoch 
        ########## 我加的程式碼 ########## 
